...
public class BlueMapIntegration {
    ...
    private static void registerOnEnable() throws Exception {
        Consumer<Object> callback = api -> { blueMapAPI = api; enabled = true; try { updateAllMarkers(); } catch (Exception e) {} };
        ...
        try {
            ...
            for (Object map : maps) {
                ...
                for (Town town : NationsData.getAllTowns()) {
                    if (town.getNationName() == null) try { drawStandaloneTown(town, tMarkers); } catch (Exception e) {}
    ...
    private static void drawNationTerritory(Nation nation, Map<String, Object> markers) throws Exception {
        ...
        for (String tn : nation.getTowns()) { Town t = NationsData.getTown(tn); if (t != null) allChunks.addAll(t.getClaimedChunks()); }
        ...
        int i = 0;
        for (List<Point> poly : polygons) markers.put("nation_" + nation.getName() + "_" + (i++), createShapeMarker(nation.getName(), createShape(poly), fill, line, 3, ""));
    }
        ...
        for (String townName : townNames) {
            ...
            for (List<Point> poly : townPolygons) {
                ...
                if (townNames.size() == 1) {
                    ...
                    // townName (LABEL) для наведения, popup (DETAIL) для клика
                    markers.put("townborder_" + townName + "_" + (j++), createShapeMarker(townName, createShape(poly), fill, line2, 3, popup));
                } else {
                    markers.put("townborder_" + townName + "_" + (j++), createShapeMarker(townName, createShape(poly), noFill, townLine, 1, popup));
                }
            }
        }
    }
    ...
    private static void drawStandaloneTown(Town town, Map<String, Object> markers) throws Exception {
        ...
        int i = 0;
        for (List<Point> poly : polygons) markers.put("standalone_" + town.getName() + "_" + (i++), createShapeMarker(town.getName(), createShape(poly), fill, line, 2, popup));
    }

    private static void drawTownIcon(Town town, Map<String, Object> markers) throws Exception {
        if (town.getSpawnPos() == null) return;
        boolean isCapital = false;
        if (town.getNationName() != null) { Nation nation = NationsData.getNation(town.getNationName()); if (nation != null) isCapital = nation.isCapital(town.getName()); }
        double px = town.getSpawnPos().getX() + 0.5, py = town.getSpawnPos().getY() + 2.0, pz = town.getSpawnPos().getZ() + 0.5;
        String base64 = isCapital ? CAPITAL_ICON_BASE64 : TOWN_ICON_BASE64;
        int iconSize = isCapital ? 26 : 14;
        String html = "<div style=\"transform:translate(-50%,-50%);width:" + iconSize + "px;height:" + iconSize + "px;filter:drop-shadow(0 1px 3px rgba(0,0,0,0.7));cursor:pointer;z-index:1;position:relative;\">";
        if (base64 != null) html += "<img src=\"data:image/png;base64," + base64 + "\" width=\"" + iconSize + "\" height=\"" + iconSize + "\" style=\"display:block;\" />";
        else html += "<span style=\"font-size:" + (isCapital?18:8) + "px;\">" + (isCapital?"★":"•") + "</span>";
        html += "</div>";
        ...
        if (htmlUseVec3) mHtmlMarkerPosition.invoke(builder, cVector3d.newInstance(px, py, pz)); else mHtmlMarkerPosition.invoke(builder, px, py, pz);
    ...
    private static Object createShapeMarker(String label, Object shape, Object fill, Object line, int width, String detail) throws Exception {
        ...
        if (detail != null && !detail.isEmpty()) mShapeMarkerDetail.invoke(bd, detail);
    ...
    private static String buildTownPopup(Town town, Nation nation) {
        ...
        for (UUID id : town.getMembers()) memberNames.add(getPlayerName(id));

        StringBuilder sb = new StringBuilder();
        // Используем встроенные стили чтобы не зависеть от CSS
        sb.append("<div style=\"width:300px;font-family:'Segoe UI',sans-serif;font-size:13px;line-height:1.4;\">");
        sb.append("<div style=\"margin-bottom:2px;\"><span style=\"color:#b6b8bf;font-weight:bold;\">Нация: </span><span style=\"color:#ffffff;font-weight:bold;\">").append(escapeHtml(nationName)).append("</span></div>");
        sb.append("<div style=\"margin-bottom:8px;\"><span style=\"color:#b6b8bf;font-weight:bold;\">Город: </span><span style=\"color:#ffffff;font-weight:bold;\">").append(escapeHtml(townName)).append("</span></div>");
        sb.append("<div style=\"height:1px;background:#666;margin:6px 0;\"></div>");
        sb.append("<div style=\"margin-top:8px;\"><span style=\"color:#b6b8bf;font-weight:bold;\">Мэр: </span><span style=\"color:#ffd700;font-weight:bold;\">").append(escapeHtml(mayorName)).append("</span></div>");
        sb.append("<div style=\"margin-top:4px;\"><span style=\"color:#b6b8bf;font-weight:bold;\">Жители: </span><span style=\"color:#ffffff;\">").append(memberNames.isEmpty() ? "—" : String.join(", ", memberNames)).append("</span></div>");
        sb.append("</div>");
        return sb.toString();
    }

    // Вспомогательные методы (Geometry, Utils)
    private static Set<String> calcEdges(Set<ChunkPos> chunks) { Set<String> e = new HashSet<>(); for(ChunkPos c:chunks) { double x1=c.x*16,z1=c.z*16,x2=x1+16,z2=z1+16; toggleEdge(e,x1,z1,x2,z1); toggleEdge(e,x2,z1,x2,z2); toggleEdge(e,x2,z2,x1,z2); toggleEdge(e,x1,z2,x1,z1); } return e; }
    private static void toggleEdge(Set<String> s, double x1, double z1, double x2, double z2) { String f=x1+","+z1+">"+x2+","+z2, r=x2+","+z2+">"+x1+","+z1; if(s.contains(r)) s.remove(r); else s.add(f); }
    private static List<List<Point>> tracePolygons(Set<String> edges) { List<List<Point>> polys=new ArrayList<>(); Map<Point,Point> m=new HashMap<>(); for(String e:edges) { String[] p=e.split(">"); String[] a=p[0].split(","),b=p[1].split(","); m.put(new Point(Double.parseDouble(a[0]),Double.parseDouble(a[1])), new Point(Double.parseDouble(b[0]),Double.parseDouble(b[1]))); } while(!m.isEmpty()) { List<Point> poly=new ArrayList<>(); Point start=m.keySet().iterator().next(), curr=start; while(curr!=null) { poly.add(curr); Point next=m.remove(curr); if(next==null||next.equals(start)) break; curr=next; } if(poly.size()>=3) polys.add(poly); } return polys; }
    private static Object createShape(List<Point> pts) throws Exception { Object arr=java.lang.reflect.Array.newInstance(clsVector2d,pts.size()); for(int i=0;i<pts.size();i++) java.lang.reflect.Array.set(arr,i,cVector2d.newInstance(pts.get(i).x,pts.get(i).z)); return cShape.newInstance(arr); }
    private static String getPlayerName(UUID id) { if(id==null) return "—"; if(NationsData.getServer()!=null) { var p=NationsData.getServer().getPlayerList().getPlayer(id); if(p!=null) return p.getName().getString(); var pr=NationsData.getServer().getProfileCache(); if(pr!=null && pr.get(id).isPresent()) return pr.get(id).get().getName(); } return id.toString().substring(0,8)+"..."; }
    private static String escapeHtml(String t) { return t==null?"":t.replace("&","&amp;").replace("<","&lt;").replace(">","&gt;").replace("\"","&quot;"); }
    static class Point { double x,z; Point(double x,double z){this.x=x;this.z=z;} @Override public boolean equals(Object o){if(this==o)return true;if(!(o instanceof Point))return false;Point p=(Point)o;return Double.compare(p.x,x)==0&&Double.compare(p.z,z)==0;} @Override public int hashCode(){return Objects.hash(x,z);} }
}
